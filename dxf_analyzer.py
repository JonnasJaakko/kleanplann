...
def _iter_entities(msp, depth: int = 0, block_context: str = ""):
    """Yield entities together with names of their owning INSERT blocks."""
    count = 0
    for e in msp:
        count += 1
        ...
        if e.dxftype() == "INSERT":
            ...
            context = f"{block_context} {block_name}".strip()
            yield from _iter_entities(sub, depth + 1, context)
        else:
            yield e, block_context


def _is_opening_graphics(entity, block_context: str, scale: float) -> bool:
    """True for a door/window symbol, not for the wall opening itself."""
    layer = getattr(entity.dxf, "layer", "")
    context = f"{layer} {block_context}".lower()
    if any(word in context for word in OPENING_KEYWORDS):
        return True
    if entity.dxftype() == "ARC":
        try:
            radius = float(entity.dxf.radius) * scale
            sweep = (float(entity.dxf.end_angle) - float(entity.dxf.start_angle)) % 360.0
            return 0.45 <= radius <= 2.2 and 35.0 <= sweep <= 185.0
        except Exception:
            return False
    return False
...
def _collect_by_layer(msp, doc, scale: float, sagitta: float):
    """Собирает отрезки, сгруппированные по слою."""
    by_layer: Dict[str, List[LineString]] = defaultdict(list)
    total = 0
    skipped_openings = 0
    for e, block_context in _iter_entities(msp):
        layer = getattr(e.dxf, "layer", "0")
        if not _layer_is_usable(doc, layer):
            continue
        if _is_opening_graphics(e, block_context, scale):
            skipped_openings += 1
            continue
        ...
        if total > MAX_SEGMENTS:
            ...
            break
    return by_layer, skipped_openings
...
def _pick_wall_layers(by_layer: Dict[str, List[LineString]],
    ...
    # Prefer the dominant structural layer.  The former 70%-coverage rule
    # pulled smaller door, furniture and annotation layers into wall geometry.
    # Known multi-layer drawings can still pass ``wall_layers`` explicitly.
    return [ranked[0][0]]
...
def load_wall_segments(filepath: str, config: Optional[Dict] = None):
    ...
    by_layer, skipped_openings = _collect_by_layer(msp, doc, scale, sagitta)
    if not by_layer:
    ...
    # Door swings and small drafting remnants should not survive as room
    # vertices.  This stays below a typical wall thickness and can be
    # overridden per drawing through ``simplify_tol_m``.
    recommended_simplify = max(0.08, min(0.20, (thickness or 0.16) * 0.75))
    ...
    info = {
        ...
        "ignored_opening_graphics": skipped_openings,
        "recommended_simplify_tol_m": recommended_simplify,
        "warnings": warnings,
    }
...
def rooms_from_segments(segments: List[LineString], info: Dict,
    ...
    return detect_rooms(
        ...
        simplify_tol=config.get("simplify_tol_m", info.get("recommended_simplify_tol_m", 0.08)),
